    ...
    #[cfg(target_os = "linux")]
    {
        #[cfg(feature = "x11")]
        {
            ...
            attributes = attributes.with_platform_attributes(Box::new(
                WindowAttributesX11::default().with_name(
            ...
            if let Some(monitor) = monitor {
                ...
                Some(winit::dpi::Position::Physical(
                    winit::dpi::PhysicalPosition {
                        x: start.unwrap_or_default().x + centered.x,
                        y: start.unwrap_or_default().y + centered.y,
                    },
                ))
            } else {
                None
...
pub fn modifiers(modifiers: winit::keyboard::ModifiersState) -> keyboard::Modifiers {
    ...
    result.set(
        ...
        modifiers.contains(winit::keyboard::ModifiersState::META),
    );
...
pub fn key(key: winit::keyboard::Key) -> keyboard::Key {
    ...
    match key {
        winit::keyboard::Key::Character(c) if c == " " => keyboard::Key::Named(Named::Space),
